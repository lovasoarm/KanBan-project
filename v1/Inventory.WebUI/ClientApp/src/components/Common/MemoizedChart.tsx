...
const lineChartPropsAreEqual = (
  ...
  if (
    prevLabels &&
    nextLabels &&
    prevLabels.some((label, index) => label !== nextLabels[index])
  ) {
    return false;
  }
  ...
  for (let i = 0; i < prevDatasets.length; i++) {
    ...
    if (
      prevDataset.data.some((value, index) => value !== nextDataset.data[index])
    ) {
      return false;
    }
  }
...
export const MemoizedLineChart = memo<MemoizedLineChartProps>(
  ({ data, options }) => {
    console.log(
      "MemoizedLineChart rendering with data:",
      data.labels,
      "datasets length:",
  ...
  (prevProps, nextProps) => {
    ...
    const prevDatasetsStr = JSON.stringify(
      prevProps.data.datasets.map((d) => ({ label: d.label, data: d.data }))
    );
    const nextDatasetsStr = JSON.stringify(
      nextProps.data.datasets.map((d) => ({ label: d.label, data: d.data }))
    );

    const areEqual =
      prevLabelsStr === nextLabelsStr &&
      prevDatasetsStr === nextDatasetsStr &&
      prevProps.options === nextProps.options;

    if (!areEqual) {
      console.log(
        "LineChart will rerender - Labels changed:",
        prevLabelsStr !== nextLabelsStr
      );
      console.log(
        "LineChart will rerender - Datasets changed:",
        prevDatasetsStr !== nextDatasetsStr
      );
      console.log(
        "LineChart will rerender - Options changed:",
        prevProps.options !== nextProps.options
      );
    }
...
export const MemoizedDoughnutChart = memo<MemoizedDoughnutChartProps>(
  ({ data, options }) => {
